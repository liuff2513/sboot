...
<meta charset="utf-8">
    ...
    <link href="theme/css/style.min.css?v=2.2.0" rel="stylesheet">
...
if (self != top)top.location = self.location;
	...
	<body class="gray-bg" onload="loginError()">
		...
		<form role="form" name="loginForm" action="./login" method="post" class="m-t" >
			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
	                              ...
	                              <div class="form-group"><div class="login-por"><i class="login-por-i"></i></div>

<#-->
	                                <input  class="form-co"  id="exampleInputEmail1" placeholder="请输入用户名或注册邮箱" value="13245678910" type="text" name="username" required  value="${lastLoginName!''}">
<-->
	                                <input  class="form-co"  id="exampleInputEmail1" placeholder="请输入用户名或注册邮箱" value="" type="text" name="username" required value="${lastLoginName!''}">
	                            </div>
	                             <div class="form-group">

<#-->
	                                 <div class="login-code"><i class="login-code-i"></i></div><input class="form-co" value="123456" id="exampleInputEmail1" class="form-co" placeholder="请输入密码" required type="password" name="password">
<-->
	                                 <div class="login-code"><i class="login-code-i"></i></div><input class="form-co" value="" id="exampleInputEmail1" class="form-co" placeholder="请输入密码" required type="password" name="password">
	                            </div>
	                           	<#if (loginNum?has_content) && (loginNum?eval > 2) >
									<div class="form-group input-group-lg">
										<label for="verifycode" class="col-sm-2 control-label sr-only"> 验证码 </label>
										<input type="text" name="sessionVerifyName" style=" display: inline;width: 44%;  float: left;" placeholder="" id="verifycode" class="form-control">
									...
									<div id="loginError" style="display:none;" class="alert alert-warning alert-dismissible" role="alert">
										...
										<button type="button" class="close" >
											<span aria-hidden="true" onclick="$('#loginError').hide();">&times;</span><span class="sr-only" onclick="$('#loginError').hide();" >Close</span>
										</button>
										<div id = "loginErrorMsg"></div>
									</div>
				  				<p>
				                <span class="f-l"><label class=""><input type="checkbox" name="rememberMe" value="true"><span>自动登录</span></label></span>
				                <a href="./getPassword"><small>忘记密码？</small></a>
				                </p>
