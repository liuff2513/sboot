...
function reloadView(){
	...
	if(tableName=="s_role" || tableName=="s_group"){
		$("div[class='khc-sel']").hide();
		return
	}
	var functionName =  $("input[type=hidden][name='function.name']").val();
	$.ajax({
		...
		success:function(data){
			...
			//无查看视图记录 默认显示全部
			if(lastViewId==""){
				//$("ul[id='sel_ul_id']").append("<li onclick='checkedLi(this);'><span class='select-ative'><p style='display:none;'></p>所有"+functionName+"</span></li>");
			 ...
			 $("#view_customer_all").closest("ul").find("li").each(function(){
			 $(this).find("span").attr("class","");
			 });
			 $("#view_customer_all").find("span").attr("class","select-ative");
			 //$("ul[id='sel_ul_id']").append("<li onclick='checkedLi(this);'><span><p style='display:none;'></p>所有"+functionName+"</span></li>");
			 }*/


			if((null==myList || ""==myList) && (null==srList || ""==srList)){
				$("select[id='view_id']").append("<option value='' style='font-weight:bold;font-style:italic;font-family:宋体;cursor:default;'>所有"+functionName+"</option>");
				$("#view_copy").unbind("click");
				$("#view_copy").attr("style","color:#CCC;cursor:text");
				$("#view_copy").attr("onclick","null");
				return;
...
function checkedLi(obj){
	...
	var nameSpace = $("input[type=hidden][name='nameSpace']").val();
	var actionName = $("input[type=hidden][name='actionName']").val();
	$.ajax({
		...
		success:function(data){
			if(!data.length){
				...
				//window.refreshForm.action=window.location.href;
				window.refreshForm.action="../"+nameSpace+"/"+actionName+"List?nameSpace="+nameSpace+"&actionName="+actionName;
				window.refreshForm.submit();
			}
...
//<#-- 视图   复制 -->
$("a[id='view_copy']").click(function(){
	var select_viewId =  $("span[class='all-select-in']").find("p").html();
	if(select_viewId!="-1"){
		$("input[type=hidden][name=viewId]").val(select_viewId);
		$("input[type=hidden][name=viewOpName]").val("copy");
		window.listForm.action="../view/viewCreate";
		window.listForm.submit();
	}
...
//<#-- 新建视图   全部项目 -->
$("a[id='view_create']").click(function(){
	$("input[type=hidden][name=viewOpName]").val("add");
	window.listForm.action="../view/viewCreate";
	window.listForm.submit();
});
