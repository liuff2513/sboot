...
//	            	html += '					<div class="andArea">';
	            	...
	            	html += '						<input type="button" style="margin-left: 14px;" class="add_btn" value="上传附件"/>';
	            	
	            	html += '						<input style="display:none;" id="fileImage" class="file" style="margin-left: 14px;" type="file" size="30" name="cameraInput" '+multiple+'>';
	            	html += '				<div id="status_info" class="info fl">选中0张文件，共0B。</div>';
//	            	html += '				<div id="status_info" class="info fl" style="font-size:12px;color:red;">非WIFI下，文件大小不能超过150K。</div>';
//	            	html += '					</div>';
//	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">或者将文件拖到此处</span>';
					html += '			</div>';
		          //  html += '			<div class="status_bar">';
		           // html += '				<div id="status_info ml10" class="info">选中0张文件，共0B。</div>';
		          //  html += '				<div class="btns">';
		           // html += '					<div class="webuploader_pick">继续选择</div>';
		         //   html += '					<div class="upload_btn">开始上传</div>';
		         //   html += '				</div>';
		        //    html += '			</div>';
					html += '			<div id="preview" class="cba"></div>';
					html += '		</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 10485760) {//前台控制文件的大小
				...
				var fileSize = 0;
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 20971520) {//前台控制文件的大小20M
						alert('您这个"'+ file.name +'"文件大小过大(不能超过20M)');	
					} else {
						// 在这里需要判断当前所有文件中
						//fileSize += file.size;
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				//取上传的文件的类型 xie 17/4/6  使用字符串的分割和反转 方便取到后缀名
				var newStr = file.name.split("").reverse().join("");
				var file_type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = "./plugins/zyupload/control/images/fileType/";
				...
				}else if(file_type=="avi"){
					fileImgSrc = fileImgSrc + "avi.png";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						//louyong 添加if判断,超过最大值则跳过
						if(!para.onSelect(selectFiles, allFiles)) return false;  // 回调方法
